...
(define (inplace-update addr v st)
  (type-case Store st
    [mtSto () (error "not allocated address")]
    [aSto (a1 v1 st1) (if (= a1 addr)
                          (aSto addr v st1)
                          (aSto a1 v1 (inplace-update addr v st1)))]))
...
(define (interp bmfae ds st)
  (type-case BMFAE bmfae
    ...
    [setbox (t v) (interp-two t v ds st (lambda (v1 v2 st1)
                                          (v*s v2 (inplace-update (boxV-address v1) v2 st1))))]
...
(test (interp (parse '{setbox {newbox 1} 120}) (mtSub) (mtSto)) (v*s (numV 120) (aSto 1 (numV 120) (mtSto))))